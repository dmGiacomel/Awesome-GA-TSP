...
double TSPInstance::getPathCost(const std::vector<int>& path) const{
    ...
    for (size_t i{0}; i < n_cities - 1; i++){
        path_cost += adj_matrix[Utils::flattenedMatrixCoordinates(path[i],path[i+1],n_cities)];
    }
    path_cost += adj_matrix[Utils::flattenedMatrixCoordinates(path[n_cities-1],path[0],n_cities)];
    return path_cost;
}

double TSPInstance::getEdgeCost(size_t city_a, size_t city_b) const{
    return adj_matrix[Utils::flattenedMatrixCoordinates(city_a, city_b, n_cities)];
}
        ...
        adj_matrix(std::vector<double>(n_cities * n_cities, std::numeric_limits<double>::infinity()))
{
    ...
    for (size_t i = 0; i < n_cities; i++){
        for (size_t j = i + 1; j < n_cities; j++){
            ...
            adj_matrix[Utils::flattenedMatrixCoordinates(i, j, n_cities)] = random;
            adj_matrix[Utils::flattenedMatrixCoordinates(j, i, n_cities)] = random;
        }
    }
}